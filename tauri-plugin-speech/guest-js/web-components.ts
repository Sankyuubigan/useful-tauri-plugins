...
class SpeechEnginePanel extends HTMLElement {
  ...
  private render() {
    ...
    this.root.getElementById('engine_dir_input')!.addEventListener('input', (e) => {
      this.settings.engine_dir = (e.target as HTMLInputElement).value
    })
    this.root.getElementById('models_dir_input')!.addEventListener('input', (e) => {
      this.settings.models_dir = (e.target as HTMLInputElement).value
    })
  ...
  private async init() {
    ...
    ;(this.root.getElementById('engine_dir_input') as HTMLInputElement).value = this.settings.engine_dir
  ...
  private async pickDir(field: 'engine_dir' | 'models_dir') {
    ...
    if (picked && typeof picked === 'string') {
      if (field === 'engine_dir') this.settings.engine_dir = picked
      else this.settings.models_dir = picked
      ;(this.root.getElementById(field + '_input') as HTMLInputElement).value = picked
      this.saveSettings()
    }
  }
...
class SpeechVoiceStorage extends HTMLElement {
  ...
  private render() {
    ...
    this.root.getElementById('name')!.addEventListener('keydown', (e) => {
      if ((e as KeyboardEvent).key === 'Enter') this.addVoice()
    })
  ...
  private async renderList() {
    ...
    for (const v of this.voices) {
      ...
      if (v.has_avatar) {
        ...
        if (b) img.src = 'data:image/jpeg;base64,' + btoa(String.fromCharCode(...b))
      }
      row.appendChild(img)
  ...
  private async addVoice() {
    ...
    if (!name) { this.setStatus('укажите имя голоса'); return }
