import { cancelDownload, onProgress } from './index';
const STATUS_LABEL = {
    running: 'Скачивание…',
    done: 'Готово',
    error: 'Ошибка',
...
function formatEta(sec) {
    if (!Number.isFinite(sec) || sec < 0)
        return '—';
    if (sec < 60)
        return `${Math.max(1, Math.round(sec))} с`;
...
class DownloaderProgress extends HTMLElement {
    ...
    filterRows(rows) {
        ...
        if (!this.filterKinds.length && !label)
            return rows;
        const out = new Map();
        for (const [id, row] of rows) {
    ...
    render() {
        ...
        if (list)
            list.innerHTML = rowsHtml(this.filterRows(this.ctrl.rows), false);
    }
}
