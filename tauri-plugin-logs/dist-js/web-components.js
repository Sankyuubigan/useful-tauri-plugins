import { getLastLogsPath, logFront, onLogMessage, saveLogsToFile, } from './index';
/**
...
class LogsPanel extends HTMLElement {
    ...
    render() {
      ...
      <textarea id="area" readonly spellcheck="false" placeholder="Здесь появится лог приложения…"></textarea>`;
        ...
        void getLastLogsPath()
            .then((p) => {
            if (p)
                this.root.getElementById('hint').textContent = `Файл: ${p}`;
        })
            .catch(() => { });
    ...
    async onSave() {
        ...
        if (saved)
            logFront(`[logs-panel] логи сохранены в ${saved}`);
    }
}
