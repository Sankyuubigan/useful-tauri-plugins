...
class LlamaEnginePanel extends HTMLElement {
    ...
    async onVariantChange() {
        ...
        if (value !== this.applied)
            ...
            this.root.getElementById('apply').style.display = 'none';
        try {
...
class LlamaModelsPanel extends HTMLElement {
    ...
    connectedCallback() {
      ...
      <div>
        ...
        this.root.getElementById('modalOk').addEventListener('click', () => {
            ...
            this.pendingPath = '';
            if (action)
                void this.doAction(action, path);
        });
    ...
    async refresh() {
        ...
        for (const m of models) {
            ...
            if (meta) {
                if (meta.uncen) {
                    const s = span('😈', 'Без цензуры (uncensored)');
                    name.appendChild(s);
                }
                if (meta.vision) {
                    const s = span('👁️', 'Видит изображения (vision)');
                    name.appendChild(s);
                }
                if (meta.audio) {
                    const s = span('🎵', 'Понимает аудио (audio)');
                    name.appendChild(s);
                }
            }
